...
async def add_handlers(url: str, fname: Path=None, overwrite=False):
    ...
    async with owner_scoped_session() as session:
        for handler in schema.handlers:
            ...
            range_prefix, range_term = schema.context.shrink_iri(handler.target_range).split(":")
            range_voc = schema.context.expand(f"{range_prefix}:")
            target_range = await Term.ensure(session, range_term, range_voc, range_prefix)
            # TODO: Check that target_range is a valid projection, and that the target_role is part of the event's schema
            existing = await session.execute(select(EventHandler).filter_by(event_type=event_type, target_range=target_range, target_role=handler.target_role))
            if existing := existing.first():
                ...
                session.add(EventHandler(event_type=event_type, target_range=target_range, target_role=handler.target_role, language=handler.language, code_text=handler.code_text))
        await session.commit()
    return schema
...
async def add_data(data_type: str, url:str, prefix: str=None, file:Path=None, overwrite: bool=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('type', choices=['context', 'hk_schema', 'ontology', 'hk_handlers'], help='data type')
    ...
    parser.add_argument('-o', '--overwrite', action='store_true', help='overwrite existing tables')
    args = parser.parse_args()
    anyio.run(add_data, args.type, args.url, args.prefix, args.file, args.overwrite, backend_options={"debug":True})
