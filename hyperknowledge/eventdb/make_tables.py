...
def as_column(schema: ProjectionAttributeSchema) -> List[Column]:
    ...
    if schema.range == RDF.langString:
        return [
            Column(schema.name, column_type, nullable=schema.optional),
...
async def create_tables(
    schema: HkSchema, schema_struct_id: dbTopicId, overwrite: bool = False, session=None
) -> List[Base]:
    if not session:
        async with owner_scoped_session() as session:
    ...
    for cls in classes:
        ...
        r = await session.execute(
            select(ProjectionTable).filter_by(
                schema_id=schema_struct_id, tablename=tablename
            )
        )
        row = r.first()
        ...
        if overwrite or not row:
            async with escalated_session(session) as esession:
                ...
                esession.add(
                    ProjectionTable(schema_id=schema_struct_id, tablename=tablename)
                )
                db_name = str(esession.bind.engine.url).split("/")[-1]
                await esession.execute(
                    text(
                        f"GRANT SELECT ON TABLE public.{tablename} to {db_name}__client"
                    )
                )
                await esession.execute(
                    text(
                        f"GRANT SELECT ON TABLE public.{tablename} to {db_name}__member"
                    )
                )
                await esession.execute(
                    text(f"ALTER TABLE public.{tablename} ENABLE ROW LEVEL SECURITY")
                )
                await esession.execute(
                    text(
                        f"DROP POLICY IF EXISTS {tablename}_select_policy ON public.{tablename}"
                    )
                )
                await esession.execute(
                    text(
                        f"CREATE POLICY {tablename}_select_policy ON public.{tablename} FOR SELECT USING (can_read_source(source_id))"
                    )
                )
    return classes
...
async def process_schema(
    ...
    if session is None:
        async with owner_scoped_session() as session:
            s = await process_schema(
                schema, schema_json, url, prefix, overwrite, session
            )
            await session.commit()
            return s
    ...
    for name in chain(schema.eventSchemas, schema.projectionSchemas):
        ...
        await session.execute(
            insert(schema_defines_table)
            .values(term_id=term.id, schema_id=db_schema.id)
            .on_conflict_do_nothing("schema_defines_pkey")
        )
    # Ensure all the ranges
    for subschema in chain(
        schema.eventSchemas.values(), schema.projectionSchemas.values()
    ):
...
async def projection_to_db(
    session, projection: BaseModel, schema: ProjectionSchema
) -> Dict:
    db_attribs = {}
...
async def db_to_projection(
    session, db_object: Base, projectionModel: type[BaseModel], schema: ProjectionSchema
) -> BaseModel:
    proj_attribs = {}
    for attribS in schema.attributes:
        ...
        if value and attribS.range == RDF.langString:
            result = {
                "@value": value,
                "@lang": getattr(db_object, f"{attribS.name}_lang"),
            }
        ...
        proj_attribs["@type"] = projectionModel.model_fields[
            "type"
