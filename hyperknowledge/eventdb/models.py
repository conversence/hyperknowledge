...
struct_type = ENUM("hk_schema", "ontology", "ld_context", "other", name="struct_type")


event_handler_language = ENUM(
    "javascript", "wasm", "python", name="event_handler_language"
)

permission = ENUM("add_schema", "add_source", "add_handler", "admin", name="permission")
...
class Topic(Base):
    ...
    has_projections: Mapped[List[int]] = mapped_column(
        ARRAY(dbTopicId), nullable=False, server_default="{}"
    )
    ...
    async def get_by_uri(cls, session, value) -> Optional[Topic]:
        ...
        if value.startswith("urn:uuid:"):
            return await session.scalar(
                select(UUIDentifier).filter_by(value=UUIDv(value[9:]))
            )
        q = select(Vocabulary)
        term = None
        ...
        if voc:
            ...
            if term:
                return await session.scalar(
                    select(Term).filter_by(term=term, vocabulary=voc)
                )

    @classmethod
    async def ensure_url(
        cls, session, value: PydanticURIRef, ensure_term: bool = False
    ) -> Topic:
        if value.startswith("urn:uuid:"):
...
Topic.projections: Mapped[List[Topic]] = relationship(
    Topic,
    primaryjoin=foreign(Topic.has_projections).contains(remote(Topic.id)),
    viewonly=True,
)
...
class Term(Topic):
    ...
    vocabulary: Mapped[Vocabulary] = relationship(
        Vocabulary, primaryjoin=Vocabulary.id == vocabulary_id, lazy="joined"
    )
    schema: Mapped[Struct] = relationship(
        "Struct", secondary=schema_defines_table, back_populates="terms"
    )
    ...
    async def ensure(
        ...
        session,
        term: str,
        vocabulary: Union[None, str, int] = None,
        prefix: str = None,
    ) -> Term:
...
class Struct(Topic):
    ...
    hash: Mapped[BYTEA] = mapped_column(
        BYTEA, server_default="sha256(value::varchar::bytea)"
    )
    ...
    __mapper_args__ = dict(
        polymorphic_identity="struct", inherit_condition=id == Topic.id
    )

    data_schema: Mapped[Topic] = relationship(
        Topic, primaryjoin=remote(Topic.id) == foreign(data_schema_id)
    )
    as_voc: Mapped[Vocabulary] = relationship(
        Vocabulary, primaryjoin=remote(Topic.id) == foreign(is_vocab)
    )
    terms: Mapped[List[Term]] = relationship(
        Term, secondary=schema_defines_table, back_populates="schema"
    )
    ...
    async def ensure(
        ...
        id_ = await session.scalar(
            ensure_struct(
                func.cast(value, JSONB),
                func.cast(subtype, struct_type),
                url,
                prefix,
                schema_type,
            )
        ...
        return await session.get(
            cls, id_, options=(joinedload(cls.data_schema), joinedload(cls.as_voc))
        )
...
class BinaryData(Topic):
    ...
    __mapper_args__ = {"polymorphic_identity": "binary_data"}
...
class Agent(Base):
    ...
    permissions: Mapped[List[permission]] = mapped_column(
        ARRAY(permission), server_default="{}"
    )
    created: Mapped[datetime] = mapped_column(
        TIMESTAMP, server_default="now() AT TIME ZONE 'UTC'"
    )
    last_login: Mapped[datetime] = mapped_column(TIMESTAMP)
    last_login_email_sent: Mapped[datetime] = mapped_column(TIMESTAMP)

    source_permissions: Mapped[List[AgentSourcePermission]] = relationship(
        "AgentSourcePermission", back_populates="agent"
    )
    source_selective_permissions: Mapped[List[AgentSourceSelectivePermission]] = (
        relationship("AgentSourceSelectivePermission", back_populates="agent")
    )
    processors: Mapped[List[EventProcessor]] = relationship(
        "EventProcessor", back_populates="owner"
    )
...
class Source(Vocabulary):
    ...
    id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Vocabulary.id), primary_key=True
    )
    ...
    included_source_ids: Mapped[List[int]] = column_property(
        select(func.array_agg(source_inclusion_table.c.included_id))
        .filter(source_inclusion_table.c.including_id == id)
        .scalar_subquery()
    )
    included_sources: Mapped[List[Source]] = relationship(
        "Source",
        secondary=source_inclusion_table,
        primaryjoin=id == source_inclusion_table.c.including_id,
        secondaryjoin=source_inclusion_table.c.included_id == id,
        back_populates="including_sources",
    )
    including_sources: Mapped[List[Source]] = relationship(
        "Source",
        secondary=source_inclusion_table,
        primaryjoin=id == source_inclusion_table.c.included_id,
        secondaryjoin=source_inclusion_table.c.including_id == id,
        back_populates="included_sources",
    )

    included_source_ids_rec: Mapped[List[int]] = column_property(
        func.included_sources(foreign(id), type_=ARRAY(dbTopicId)), deferred=True
    )
    including_source_ids_rec: Mapped[List[int]] = column_property(
        func.including_sources(foreign(id), type_=ARRAY(dbTopicId)), deferred=True
    )

    included_sources_rec: Mapped[List[Source]] = relationship(
        "Source",
        viewonly=True,
        uselist=True,
        primaryjoin="Source.included_source_ids_rec.any(remote(Source.id))",
    )
    including_sources_rec: Mapped[List[Source]] = relationship(
        "Source",
        viewonly=True,
        uselist=True,
    ...
    def filter_event_query(self, query, alias=None):
        ...
        return query.filter(
            Source.included_source_ids_rec.any(target.source_id), Source.id == self.id
        )
...
class EventHandler(Base):
    ...
    event_type_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Term.id), nullable=False
    )
    target_role: Mapped[str] = mapped_column(String, nullable=False)
    target_range_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Term.id), nullable=False
    )
    language: Mapped[event_handler_language] = mapped_column(
        event_handler_language, nullable=False
    )
    ...
    target_range: Mapped[Term] = relationship(
        Term, primaryjoin=target_range_id == Term.id
    )
...
class Event(Base):
    ...
    source_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Source.id), primary_key=True
    )
    creator_id: Mapped[int] = mapped_column(dbTopicId, ForeignKey(Agent.id))
    event_type_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Term.id), nullable=False
    ...
    created: Mapped[datetime] = mapped_column(
        TIMESTAMP, primary_key=True, server_default="now()"
    )
    ...
    handlers: Mapped[List[EventHandler]] = relationship(
        EventHandler,
        primaryjoin=foreign(event_type_id) == remote(EventHandler.event_type_id),
        viewonly=True,
        uselist=True,
    )

    including_source_ids_rec = column_property(
        func.including_sources(foreign(source_id), type_=ARRAY(dbTopicId)),
        deferred=True,
    )

    included_in_sources: Mapped[List[Source]] = relationship(
        Source,
        viewonly=True,
        uselist=True,
...
class EventProcessor(Base):
    ...
    owner_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Agent.id), nullable=False
    )
    source_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Source.id), nullable=True
    )
    last_event_ts: Mapped[datetime] = mapped_column(TIMESTAMP)
...
class ProjectionTable(Base):
    ...
    schema_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Struct.id), primary_key=True
    )
...
class AgentSourcePermission(Base):
    ...
    agent_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Agent.id), primary_key=True
    )
    source_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Source.id), primary_key=True
    )
...
class AgentSourceSelectivePermission(Base):
    ...
    event_type_id: Mapped[int] = mapped_column(
        dbTopicId, ForeignKey(Term.id), primary_key=True
    )
    is_request: Mapped[bool] = mapped_column(Boolean, server_default="false")

    agent: Mapped[Agent] = relationship(
        Agent, back_populates="source_selective_permissions"
    )
    source: Mapped[Source] = relationship(Source)
...
class ProjectionMixin:
    ...
    def last_event(cls) -> Mapped[Event]:
        return relationship(
            Event,
            primaryjoin=(remote(Event.source_id) == foreign(cls.source_id))
...
async def delete_data(session):
    ...
    await session.execute(
        delete(Topic.__table__).filter(
            Topic.__table__.c.id.in_(select(Struct.id).filter_by(subtype="hk_schema"))
        )
    )
