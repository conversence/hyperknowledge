...
class JsonLdModel(BaseModel):
    ...
    def __init__(__pydantic_self__, **data: Any) -> None:
        ...
        __pydantic_self__.__pydantic_validator__.validate_python(
            data, self_instance=__pydantic_self__, context=dict(ctx=context)
        )
...
def validators_for_schema(schema: EventSchema):
    ...
    for ev_schema in schema.attributes:
        if validator := validators.get(as_tuple_or_scalar(ev_schema.range)):
            validators[f"validate_{ev_schema.name}"] = field_validator(ev_schema.name)(
                validator
            )
    return validators
...
def models_from_schema(schema: HkSchema) -> Dict[QName, BaseModel]:
    ...
    return {
        f"{prefix}:{sschema.name}": model_from_schema(sschema, prefix)
        for sschema in schema.eventSchemas.values()
    } | {
        f"{prefix}:{sschema.name}": model_from_schema(sschema, prefix)
        for sschema in schema.projectionSchemas.values()
    }
...
def getEventModel() -> type[GenericEventModel]:
    ...
    if EVENT_MODEL is None:
        if types := tuple(
            m for (s, m) in KNOWN_MODELS.values() if not isinstance(s, ProjectionSchema)
        ):
...
def getProjectionSchemas() -> Dict[PydanticURIRef, Tuple[ProjectionSchema, BaseModel]]:
    ...
    return {
        s.type: (s, m)
        for (s, m) in KNOWN_MODELS.values()
        if isinstance(s, ProjectionSchema)
    }
