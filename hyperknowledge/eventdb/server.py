...
from .schemas import (
    ...
    AgentModelOptional,
    KNOWN_MODELS,
    AgentSourceSelectivePermissionModel,
    AgentSourcePermissionModel,
    AgentSourceSelectivePermissionModelOptional,
    AgentSourcePermissionModelOptional,
)
from .models import (
    Source,
    Event,
    ...
    EventHandler,
    Agent,
    AgentSourceSelectivePermission,
    AgentSourcePermission,
    schema_defines_table,
)
from .make_tables import (
    read_existing_projections,
    KNOWN_DB_MODELS,
    process_schema,
    db_to_projection,
)
from .auth import get_token, Token, CurrentAgentType, CurrentActiveAgentType
...
async def populate_app(app: FastAPI, initial=False):
    ...
    for i, r in reversed(list(enumerate(app.router.routes))):
        if r.path == "/source/{source_name}/events" or r.path.startswith(
            "/source/{source_name}/topic/{entity_id}/"
        ):
    ...
    async def post_event(
        ...
        async with agent_session(current_agent) as session:
            ...
            if not event_type.schema:
                raise HTTPException(
                    400, f"Event type {event.data.type} has no schema defined"
                )
            ...
            try:
                for attrib in event_schema.attributes:
                    if attrib.create and not getattr(event.data, attrib.name):
                        ...
                        range_prefix, range_term = hk_schema.context.shrink_iri(
                            attrib.range
                        ).split(":")
                        resourceType = await Term.ensure(
                            session, range_term, None, range_prefix
                        )
    ...
    def define_projection_getter(topic, schema, db_model, pyd_model):
        @app.get(
            "/source/{source_name}/topic/{entity_id}/"
            + f"{topic.vocabulary.prefix}:{schema.name}"
        )
        async def get_projection(
            ...
            async with agent_session(current_agent) as session:
                q = (
                    select(db_model)
                    .filter_by(obsolete=None)
                    .join(UUIDentifier)
                    .filter_by(value=entity_id)
                    .join(Source, db_model.source_id == Source.id)
                    .filter_by(local_name=source_name)
                )
                projection = await session.execute(q)
                if projection := projection.first():
                    model = await db_to_projection(
                        session, projection[0], pyd_model, schema
                    )
                    response.headers["Link"] = (
                        f'<{request.base_url}schema/{schema_id}/context>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"'
                    )
                    return model
    ...
    async with owner_scoped_session() as session:
        for schema_type, (schema, pyd_model) in getProjectionSchemas().items():
            ...
            schema_id = await session.scalar(
                select(schema_defines_table.c.schema_id).filter(
                    schema_defines_table.c.term_id == topic.id
                )
            )
...
async def login_for_access_token(
    ...
    token = await get_token(
        form_data.username, form_data.password, ACCESS_TOKEN_EXPIRE_MINUTES * 60
    )
    if not token:
        raise HTTPException(
...
async def read_agents_me(current_agent: CurrentActiveAgentType):
    return current_agent


@app.post("/agents")
async def add_agent(
    model: AgentModelWithPw, response: Response, current_agent: CurrentAgentType
) -> None:
    async with agent_session(current_agent) as session:
        # TODO: Password integrity rules
        await session.scalar(
            func.create_agent(
                model.email, model.passwd, model.username, model.permissions
            )
        )
        await session.commit()
        response.status_code = status.HTTP_201_CREATED
...
async def get_remote_source(
    source_id: int, current_agent: CurrentAgentType
) -> RemoteSourceModel:
    async with agent_session(current_agent) as session:
...
async def add_remote_source(
    model: RemoteSourceModel, response: Response, current_agent: CurrentActiveAgentType
) -> RemoteSourceModel:
...
async def get_local_source(
    source_name: str, current_agent: CurrentAgentType
) -> LocalSourceModel:
    async with agent_session(current_agent) as session:
        source = await session.execute(
            select(Source)
            .filter_by(local_name=source_name)
            .options(joinedload(Source.creator))
        )
...
async def add_local_source(
    ...
    async with agent_session(current_agent) as session:
        ...
        if model.included_source_ids:
            can_read = await session.execute(
                select(
                    and_(
                        *[
                            func.can_read_source(source_id)
                            for source_id in model.included_source_ids
                        ]
                    )
                )
            )
        ...
        source = await Source.ensure(
            session,
            uri,
            model.local_name,
            model.public_read,
            model.public_write,
            model.selective_write,
            model.included_source_ids,
        )
        await session.commit()
        response.status_code = status.HTTP_201_CREATED
...
async def get_local_source_permissions(
    source_name: str, current_agent: CurrentActiveAgentType
) -> List[Union[AgentSourceSelectivePermissionModel, AgentSourcePermissionModel]]:
    async with agent_session(current_agent) as session:
        r = await session.execute(select(Source).filter_by(local_name=source_name))
        r = r.first()
        if not r:
        ...
        q = (
            select(AgentSourceSelectivePermission)
            .filter_by(source_id=source.id)
            .options(
        ...
        assps = [
            AgentSourceSelectivePermissionModel.model_validate(assp)
            for (assp,) in assps
        ]
...
async def add_local_source_permissions(
    source_name: str,
    permission: Union[
        AgentSourceSelectivePermissionModelOptional, AgentSourcePermissionModelOptional
    ],
    current_agent: CurrentActiveAgentType,
    request: Request,
    response: Response,
    ...
    async with agent_session(current_agent) as session:
        ...
        if permission.agent and permission.agent != current_agent.username:
            r = await session.execute(
                select(Agent).filter_by(username=permission.agent)
            )
            r = r.first()
            if not r:
...
async def update_local_source_permissions(
    ...
    async with agent_session(current_agent) as session:
        ...
        if getattr(permission, "event_type", None):
            ...
            r = await session.execute(
                select(AgentSourceSelectivePermission).filter_by(
                    source=source, agent=agent, event_type=event_type
                )
            )
            r = r.first()
            if not r:
            ...
            r = await session.execute(
                select(AgentSourcePermission).filter_by(source=source, agent=agent)
            )
            r = r.first()
            if not r:
                raise HTTPException(status_code=404, detail="Permission does not exist")
    ...
    async with agent_session(current_agent) as session:
        ...
        if getattr(permission, "event_type", None):
            ...
            await session.execute(
                delete(AgentSourceSelectivePermission).filter(
                    AgentSourceSelectivePermission.source_id == source.id,
                    AgentSourceSelectivePermission.agent_id == agent.id,
                    AgentSourceSelectivePermission.event_type_id == event_type.id,
                )
            )
        else:
            await session.execute(
                delete(AgentSourcePermission).filter(
                    AgentSourcePermission.source_id == source.id,
                    AgentSourcePermission.agent_id == agent.id,
                )
            )
        await session.commit()
...
async def add_schema(
    ...
    if not base:
        raise HTTPException(
            status_code=400, detail="The schema needs to have a _base or vocab"
        )
    prefix = ctx._prefixes[base]
    if not prefix:
        raise HTTPException(
            status_code=400, detail="The schema base needs to have a prefix"
        )
    async with agent_session(current_agent) as session:
        db_schema = await process_schema(
            schema, schema_json, base, prefix, session=session
        )
        await session.commit()
...
@app.get("/schema")
async def get_schema_ids_and_prefixes(
    current_agent: CurrentAgentType,
) -> List[Tuple[int, str]]:
    async with agent_session(current_agent) as session:
        r = await session.execute(
            select(Struct.id, Vocabulary.prefix)
            .filter_by(subtype="hk_schema")
            .join(Struct.as_voc)
        )
        return r.all()
...
async def get_schema_by_prefix(
    prefix: str, current_agent: CurrentAgentType
) -> List[HkSchema]:
    async with agent_session(current_agent) as session:
        r = await session.execute(
            select(Struct)
...
async def get_schema_component(
    prefix: str, component: str, current_agent: CurrentAgentType
) -> EventSchema:
    async with agent_session(current_agent) as session:
        s: Optional[Struct] = await session.scalar(
            ...
            .filter_by(subtype="hk_schema")
            .join(Struct.terms)
            .filter_by(term=component)
            .join(Term.vocabulary)
            .filter_by(prefix=prefix)
        )
        if s:
            m = HkSchema.model_validate(s.value)
...
async def get_schema_component_js(
    prefix: str, component: str, current_agent: CurrentAgentType
) -> Dict:
    async with agent_session(current_agent) as session:
        ...
        if s:
            m = HkSchema.model_validate(s.value)
            c_schema = m.eventSchemas.get(component) or m.projectionSchemas.get(
                component
            )
...
async def get_entity(
    source_name: str, entity_id: UUID, current_agent: CurrentAgentType
) -> EntityTopicSchema:
    async with agent_session(current_agent) as session:
        base_topic: UUIDentifier = await session.scalar(
            select(UUIDentifier).filter_by(value=entity_id)
        )
        if not base_topic:
            raise HTTPException(404, "No such topic")
        terms = await session.execute(
            select(Term).filter(Term.id.in_(base_topic.has_projections))
        )
        ...
        qs = [
            select(literal_column(f"'{terms[tid].qname}'"))
            .select_from(m)
            .filter_by(id=base_topic.id, source_id=source.id)
            for (tid, m) in db_model_by_term.items()
        ]
        ...
        if False:
            ...
            for projection_schema_term_uri in terms:
                ...
                if not projection_cls:
                    continue
                (schema_def, schema_model) = KNOWN_MODELS.get(
                    projection_schema_term_uri, (None, None)
                )
                if not schema_model:
                    continue
                projection = await session.scalar(
                    select(projection_cls).filter_by(
                        obsolete=None, id=base_topic.pid, source_id=source.id
                    )
                )
                if projection:
...
async def get_contexts(current_agent: CurrentAgentType) -> Dict[int, PydanticURIRef]:
    async with agent_session(current_agent) as session:
        contexts = await session.execute(
            select(Struct)
            .filter_by(subtype="ld_context")
            .options(joinedload(Struct.as_voc))
        )
...
async def get_context(
    ctx_id: int, response: Response, current_agent: CurrentAgentType
) -> Dict:
    async with agent_session(current_agent) as session:
        ...
        response.headers["Link"] = (
            f'<{context.as_voc.uri}>; rel="source"; type="application/ld+json"'
        )
...
async def add_context(
    data: ContextData, response: Response, current_agent: CurrentActiveAgentType
):
    url = data["url"]
    ctx = data.get("ctx")
    async with agent_session(current_agent) as session:
...
async def get_handlers(
    ...
    async with agent_session(current_agent) as session:
        handlers = await session.execute(
            select(EventHandler).options(
        ...
        return {
            eh.id: (eh.event_type.uri, eh.target_role, eh.target_range.uri)
            for (eh,) in handlers
        }
...
async def get_handler(
    handler_id: int, current_agent: CurrentAgentType
) -> EventHandlerSchema:
    async with agent_session(current_agent) as session:
        ...
        data = dict(
            handler.__dict__,
            event_type=handler.event_type.uri,
            target_range=handler.target_range.uri,
        )
...
async def add_handlers(
    handlers: EventHandlerSchemas,
    response: Response,
    current_agent: CurrentActiveAgentType,
) -> List[EventHandlerSchema]:
    if not current_agent.has_permission("add_handler"):
        raise HTTPException(401)
    db_handlers = []
    async with agent_session(current_agent) as session:
        for handler in handlers.handlers:
            event_type = await Term.ensure_url(
                session, handlers.context.expand(handler.event_type), True
            )
            target_range = await Term.ensure_url(
                session, handlers.context.expand(handler.target_range), True
            )
