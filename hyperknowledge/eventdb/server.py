...
from .schemas import (
    ...
    models_from_schemas, HkSchema, getProjectionSchemas, EntityTopicSchema)
...
from .auth import (
    authenticate_agent, create_access_token, get_current_agent, get_current_active_agent, Token, get_agent_by_username)
...
async def populate_app(app: FastAPI, initial=False):
    ...
    async def get_events(source_name: str, limit: int=50, after: datetime=None) -> List[EventSchema]:
    ...
    async def post_event(
            ...
            request: Request,
            response: Response,
            current_agent: Annotated[AgentModel, Depends(get_current_active_agent)]
          ) -> EventSchema:
    ...
    def define_projection_getter(topic, schema, db_model, pyd_model):
        ...
        async def get_projection(source_name: str, entity_id: str, request: Request, response: Response) -> pyd_model:
            async with Session() as session:
...
@app.post("/agents")
async def add_agent(model: AgentModelWithPw, response: Response) -> None:
    async with Session() as session:
...
async def get_remote_sources() -> Dict[int, PydanticURIRef]:
    async with Session() as session:
...
async def get_remote_source(source_id: int) -> RemoteSourceModel:
    async with Session() as session:
...
async def add_remote_source(model: RemoteSourceModel, response: Response):
    async with Session() as session:
...
@app.get("/source")
async def get_local_sources() -> Dict[str, PydanticURIRef]:
    async with Session() as session:
        r = await session.execute(select(Source).filter_by(local_name!=None))
...
async def get_local_source(source_name: str) -> LocalSourceModel:
    async with Session() as session:
...
@app.post("/source")
async def add_local_source(model: LocalSourceModel, request: Request, response: Response):
    async with Session() as session:
...
@app.post("/schema")
async def add_schema(schema: HkSchema, request: Request, response: Response):
    ...
    db_schema = await process_schema(schema, schema_json, base, prefix)
...
@app.get("/schema")
async def get_schema_ids_and_prefixes() -> List[Tuple[int, str]]:
    async with Session() as session:
...
async def get_schema(schema_id: int) -> HkSchema:
    async with Session() as session:
...
async def get_schema_by_prefix(prefix: str) -> List[HkSchema]:
    async with Session() as session:
...
async def get_schema_context(schema_id: int) -> Dict:
    async with Session() as session:
        s = await session.get(Struct, schema_id)
        if s:
            if s.subtype != 'hk_schema':
...
async def get_schema_context(prefix: str, component:str) -> EventSchema:
    async with Session() as session:
...
async def get_schema_context(prefix: str, component:str) -> Dict:
    async with Session() as session:
        r = await session.execute(
            select(Struct).filter_by(subtype='hk_schema'
                ).join(Struct.terms).filter_by(term=component
...
async def get_entity(source_name: str, entity_id: str) -> EntityTopicSchema:
    async with Session() as session:
        # WTF?
...
@app.get("/context")
async def get_contexts() -> Dict[int, PydanticURIRef]:
    async with Session() as session:
...
async def get_context(ctx_id: int, response: Response) -> Dict:
    async with Session() as session:
...
@app.post("/context")
async def add_context(data: ContextData, response: Response) -> int:
    ctx = data['ctx']
    url = data['url']
    async with Session() as session:
...
@app.get("/handler")
async def get_handlers() -> Dict[int, Tuple[str, str, str]]:
    async with Session() as session:
...
async def get_handler(handler_id: int) -> EventHandlerSchema:
    async with Session() as session:
...
@app.post("/handler")
async def add_handlers(handlers: EventHandlerSchemas, response: Response) -> List[int]:
    db_handlers = []
    async with Session() as session:
