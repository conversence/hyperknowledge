...
async def test_add_schema(client, clean_tables, simple_schema, hk_events_ctx):
    ...
    response = await client.post('/context', json=dict(url='https://hyperknowledge.org/schemas/hyperknowledge_events.jsonld', ctx=hk_events_ctx))
    assert response.status_code == 201
    # Add the schemas
    response = await client.post('/schema', json=simple_schema)
    assert response.status_code == 201
...
async def test_add_event(client, clean_tables, simple_schema, handlers):
    ...
    response = await client.post('/handler', json=handlers)
    assert response.status_code == 201, response.json()
    agent = AgentModelWithPw(username='admin', email='admin@example.com', passwd='admin', is_admin=True)
    # Create a user
    ...
    response = await client.get(f"/source/{source.local_name}/topic/{doc_id[9:]}/ex:document")
    assert response.status_code == 200, response.json()
    result = response.json()
